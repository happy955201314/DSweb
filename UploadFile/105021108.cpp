class Student{
    public:
        void makeMove(int Record[5][6], int Max[5][6], Color color[5][6], Color inputColor){
            ...
            if(color[4][1] == Black && color[2][4]== inputColor){x = 2;y = 4; return;}
            ...
            for(i=0;i<5;i++) for(j=0;j<6;j++){
                if(color[i][j] == inputColor && Record[i][j] < Max[i][j] -1){
                    ...
                    if(k == false){
                        ...
                        else if(color[i-1][j] == rivalColor && (Max[i-1][j] - Record[i-1][j]) == (Max[i][j] - Record[i][j])){
                            if((color[i-2][j] == inputColor   && Record[i-2][j]   == Max[i-2][j]   -1)||
                               (color[i-1][j+1] == inputColor && Record[i-1][j+1] == Max[i-1][j+1] -1)||
                               (color[i-1][j-1] == inputColor && Record[i-1][j-1] == Max[i-1][j-1] -1)){
                                k = true;
                        ...
                        else if(color[i][j+1] == rivalColor && (Max[i][j+1] - Record[i][j+1]) == (Max[i][j] - Record[i][j])){
